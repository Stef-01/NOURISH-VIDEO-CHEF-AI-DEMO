...
interface CheckpointConfig {
    ...
    bgImage: string;
    aiDetected?: string; // The specific issue AI thinks it sees
    isSaltCheck?: boolean;
}
...
export const Checkpoint: React.FC<ScreenProps & { screenId: ScreenId }> = ({ onNavigate, screenId }) => {
    ...
    const configs: Record<string, CheckpointConfig> = {
        'CHECK_ONIONS': {
            ...
            aiChips: ['Still raw', 'Sticking', 'Burning - decrease heat', 'Just right'],
            aiDetected: 'Burning - decrease heat',
            aiRemedies: {
                'Still raw': 'Raise heat slightly to medium-high. Cook for 2 more minutes.',
        ...
        'CHECK_OIL': {
            ...
            aiChips: ['Too hot', 'Smoking - decrease heat', 'Just right', 'Not hot enough'],
            aiDetected: 'Too hot',
            aiRemedies: {
                ...
                'Not hot enough': 'Keep on medium heat. Wait until oil shimmers (about 30 more seconds).'
            },
        ...
        'CHECK_PALAK': {
            ...
            aiDetected: 'Too thick - add water',
            aiRemedies: {
    ...
    const handleCapture = () => {
        if (config.isSaltCheck) {
            setShowScan(true);
        } else {
            setCaptured(true);
            if (config.aiDetected) {
                setExpandedChip(config.aiDetected);
            }
        }
    };
        ...
        return (
            <div className="h-full flex flex-col bg-black relative">
                ...
                <button
                    ...
                    className="absolute bottom-10 left-6 right-6 h-14 bg-white text-black font-bold rounded-full z-20 shadow-xl hover:scale-[1.05] active:scale-[0.95] transition-all duration-300 animate-[popIn_0.6s_cubic-bezier(0.34,1.56,0.64,1)_0.5s_both]"
                >
                    See results
                </button>
            </div>
                ...
                <div className="w-full bg-black/40 backdrop-blur-xl rounded-t-[32px] p-6 pb-10 border-t border-white/10">
                    {!captured ? (
                        <div className="space-y-4">
                            <button onClick={handleCapture} className="w-full py-4 rounded-[20px] bg-white text-black font-bold text-[17px] flex items-center justify-center gap-3 shadow-lg hover:bg-gray-50 hover:scale-[1.02] active:scale-[0.98] transition-all duration-200">
                                <IconCamera size={22} />
                                {config.isSaltCheck ? 'Scan label' : 'Take photo'}
                            </button>
                            <button onClick={handleCapture} className="w-full py-4 rounded-[20px] bg-white/10 text-white font-bold text-[17px] flex items-center justify-center gap-3 backdrop-blur-md border border-white/10 hover:bg-white/20 hover:scale-[1.02] active:scale-[0.98] transition-all duration-200">
                        ...
                        <div className="animate-[scaleIn_0.4s_cubic-bezier(0.34,1.56,0.64,1)]">
                            ...
                            <div className="flex gap-2 mb-4 bg-white/10 rounded-full p-1 animate-[slideUp_0.5s_cubic-bezier(0.34,1.56,0.64,1)_0.1s_both]">
                                <button
                                    ...
                                    className={`flex-1 py-2 rounded-full text-[13px] font-bold transition-all ${activeTab === 'ai'
                                ...
                                <button
                                    ...
                                    className={`flex-1 py-2 rounded-full text-[13px] font-bold transition-all ${activeTab === 'taste'
                                        ? 'bg-nourish-gold text-black shadow-lg'
                                        : 'text-white/70 hover:text-white'
                                        }`}
                                >
                            ...
                            <div className="space-y-3">
                                {activeTab === 'ai' ? (
                                    ...
                                    config.aiChips.map((chip, index) => {
                                        ...
                                        const isAiDetected = config.aiDetected === chip;
                                        return (
                                            <div
                                                key={chip}
                                            ...
                                            >
                                                <button
                                                    ...
                                                    className={`w-full px-5 py-3 rounded-2xl border font-medium transition-all text-left flex justify-between items-center ${isAiDetected
                                                        ? 'bg-nourish-gold/20 border-nourish-gold text-white shadow-[0_0_15px_rgba(212,168,75,0.3)] scale-[1.02]'
                                                        : 'bg-white/5 border-white/10 text-white/50 hover:bg-white/10 hover:text-white/80'
                                                        }`}
                                                >
                                                    <span>{chip}</span>
                                                    {isAiDetected && (
                                                        <span className="text-[11px] font-bold bg-nourish-gold text-black px-2 py-0.5 rounded uppercase tracking-wider">AI Detected</span>
                                                    )}
                                                </button>
